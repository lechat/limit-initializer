...
const (
	defaultAnnotation      = "initializer.kubernetes.io/limits"
	defaultInitializerName = "limits.initializer.kubernetes.io"
	defaultConfigmap       = "limits-initializer"
...
func initializeDeployment(deployment *v1beta2.Deployment, c *config, clientset *kubernetes.Clientset) error {
	if deployment.ObjectMeta.GetInitializers() != nil {
		...
		if initializerName == pendingInitializers[0].Name {
			...
			for i, cont := range initializedDeployment.Spec.Template.Spec.Containers {
				log.Printf("before resource limits for '%s' container: '%s'", cont.Name, cont.Resources)
				if len(cont.Resources.Limits) == 0 {
					log.Printf("Adding resource limits to '%s' container in deployment '%s'", cont.Name, initializedDeployment.Name)
					initializedDeployment.Spec.Template.Spec.Containers[i].Resources.Limits = c.Resources.Limits
				}
				if len(cont.Resources.Requests) == 0 {
					log.Printf("Adding resource requests to '%s' container in deployment '%s'", cont.Name, initializedDeployment.Name)
					initializedDeployment.Spec.Template.Spec.Containers[i].Resources.Requests = c.Resources.Requests
				}
			...
			log.Printf("%s", initializedDeployment.Spec.Template.Spec.Containers[0].Resources)
